...
echo "backend_repo=$(git -C /opt/biddingflow/backend rev-parse HEAD 2>/dev/null || true)"
echo "backend_deployed=$(cat /var/lib/biddingflow/backend.sha 2>/dev/null || true)"
echo "frontend_repo=$(git -C /opt/biddingflow/frontend rev-parse HEAD 2>/dev/null || true)"
