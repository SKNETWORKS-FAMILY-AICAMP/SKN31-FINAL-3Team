...
from erp_client import erp_post, erp_submit, erp_send_email, ERPNextAPIError
...
def create_and_send_po(rfq_name, supplier_id):
    ...
    try:
        ...
        recipient_email = f"{supplier_id}@example.com" 
        subject = f"발주서(PO) 안내 - {po_name}"
        content = f"안녕하세요. 귀하의 견적을 바탕으로 발주서({po_name})를 송부합니다."
...
if __name__ == "__main__":
    ...
    parser.add_argument("supplier_id", help="선정된 공급사 ID (예: SUP-0012)")
