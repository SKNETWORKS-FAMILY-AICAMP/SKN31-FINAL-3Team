...
사용법 (레포 루트에서):
    python -m backend_logic2.workflow.process_repl MAT-MR-2026-00286
    python -m backend_logic2.workflow.process_repl MAT-MR-2026-00286 --thread my-test-1
...
def _handle_select_rfq_targets(payload: dict) -> dict:
    ...
    if not candidates:
        print("공급사 후보가 없습니다. 수동 공급사를 입력합니다.")
        name = _ask("  업체명: ")
        email = _ask("  이메일: ")
        deadline = _ask("  견적 마감일(YYYY-MM-DD, 엔터로 기본값 사용): ")
        answer = {
            "suppliers": [name],
            "supplier_updates": [{"name": name, "email": email}],
        }
        if deadline:
            answer["quotation_deadline"] = deadline
        return answer
...
def _handle_po_approval(payload: dict) -> dict:
    print(
        f"PO 승인 대상: {payload.get('selected_supplier')} "
        f"(mode={payload.get('purchase_mode')}, rfq={payload.get('rfq_name')})"
    )
    choice = _ask("[a]pprove / [r]eject (?=원본보기): ")
    if choice == "?":
        _print_raw(payload)
...
def _handle_order_start(payload: dict) -> dict:
    print(f"발주 시작 대상: {payload.get('selected_supplier')} (RFQ: {payload.get('rfq_name')})")
    choice = _ask("PO 승인 단계로 이동할까요? [y/N]: ")
...
def main() -> None:
    if len(sys.argv) < 2:
        print("사용법: python -m backend_logic2.workflow.process_repl <MR이름> [--thread 이름]")
        return
