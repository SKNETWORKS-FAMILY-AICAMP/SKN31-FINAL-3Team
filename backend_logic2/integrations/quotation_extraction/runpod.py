"""RunPod Serverless adapter for quotation image/PDF extraction.
...
class RunPodQuotationConfig:
    ...
    prompt_version: str = "qwen35-quotation-json-v1"
    ...
    def from_env(cls) -> "RunPodQuotationConfig":
        ...
        return cls(
            ...
            max_document_bytes=_positive_int(
                "RUNPOD_QUOTATION_MAX_DOCUMENT_BYTES", str(25 * 1024 * 1024)
            ),
            ...
            prompt_version=(
                os.getenv(
                    ...
                    "qwen35-quotation-json-v1",
                ).strip()
                or "qwen35-quotation-json-v1"
            ),
        )
    ...
    def _documents(self, prepared: Any) -> list[dict[str, str]]:
        ...
        if not inputs:
            raise RunPodQuotationParserError(
                "RunPod 견적 추출은 현재 이미지 또는 PDF 입력만 지원합니다."
            )
    ...
    def _prompt(
        ...
        if specification_keys:
            additions.append(
                ...
                + "\n이미지에 실제 값이 있는 키만 specifications에 기록하세요."
            )
        if reflection_errors:
            additions.append(
                "[이전 검토에서 확인된 오류]\n"
                + "\n".join(f"- {value}" for value in reflection_errors)
                + "\n위 오류를 문서 이미지와 다시 대조해 교정하세요."
            )
